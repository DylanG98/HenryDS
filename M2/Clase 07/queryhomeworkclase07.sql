...
select i.nombre, i.apellido, c.idCarrera from instructor i inner join cohorte c on  c.idCarrera = 1;
...
select nombre,apellido from alumno where idCohorte = 1235 and fechaIngreso > '2019-01-01' and fechaIngreso < '2020-01-01';
select nombre,apellido from alumno where idCohorte = 1235 and fechaIngreso between  '2019-01-01'and '2020-01-01';

delete from carrera where idCarrera > 2;
delete from instructor where idInstructor > 8;


select a.nombre, a.apellido, c.nombre from alumno a inner join cohorte f on a.idCohorte = f.idCohorte inner join carrera c on f.idCarrera = c.idCarrera;
...
select count(nombre) from carrera;   #cant carrera en Henry  1

select count(idAlumno) from alumno; #cant alumnos 2

select idCohorte,count(idAlumno) from alumno group by idCohorte; #cant alumnos por cohorte   3
...
############################# para corroborar que esté bien la query

select CONCAT(nombre, " ",apellido)  as 'Nombre Completo', fechaIngreso from alumno order by fechaIngreso desc;   #4

select CONCAT(nombre, " ",apellido)  as 'Nombre Completo', fechaIngreso from alumno order by fechaIngreso asc limit 1;    #  5  & 6

select CONCAT(nombre, " ",apellido)  as Nombre_Completo, fechaIngreso from alumno order by fechaIngreso desc limit 1;  #7

select  year(fechaIngreso) as Año, count(year(fechaIngreso)) as Ingresantes from alumno group by Año order by Año asc;  #8     no toma la nueva refencia año cuando quiero ponerle nombre a la segunda columna, si para ordenar

select weekofyear(fechaIngreso) as 'Ingresantes por Semana', year(fechaIngreso) as Año from Alumno order by Año asc;  #9

select year(fechaIngreso) as Año, count(*) from alumno group by year(fechaIngreso) having count(*) > 20;  # 10
...
select nombre, apellido, timestampdiff(year,fechaNacimiento,curdate()) as 'Edad Instructor' from instructor;      #11

select nombre,apellido,fechaNacimiento from instructor;


select date_add("1998-05-25", interval 3 year) ;   ### adelanta o vuelve el tiempo que le pidas
...
select nombre, apellido, timestampdiff(year, fechaNacimiento, curdate()) as Edad from alumno where timestampdiff(year, fechaNacimiento, curdate()) > 31 order by Edad asc;
