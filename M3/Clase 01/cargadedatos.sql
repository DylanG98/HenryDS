...
alter table empleado rename column IDEmpleado to IdEmpleado;
alter table calendario rename column id to IdCalendario;
alter table empleado drop column IdEmpleado;
alter table empleado drop column IdSucursal;
alter table empleado add IdSucursal int (11);
...
select IdEmpleado from empleado group by IdEmpleado having count(*) > 1;  ######################## utilizo esto para buscar valores repetidos


#################### CADA TABLA TIENE QUE TENER SU PRIMARY KEY ############### 

ALTER TABLE empleado ADD IdEmpleados INT NOT NULL AUTO_INCREMENT PRIMARY KEY;   ################ hago columnas primary key auto incrementales, borro las id anteriores 
alter table venta add primary key(IdVenta);                                     ################# no es necesario que sea auto_increment si no se repiten los values, solo asigno pk a una column existente

#####################################################################################################################################



################### RELACIONO COLUMNAS CON FOREIGN KEY ENTRE TABLAS QUE LO NECESITE #################

alter table empleado add foreign key (IdSucursal) REFERENCES sucursal(IdSucursal);

######################################################################################################################################

#################### BUSCO VALORES VACIOS #####################

select * from cliente where Provincia = '';
